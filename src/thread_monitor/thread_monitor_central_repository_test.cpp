...
static const bool dummy =
    ThreadMonitorCentralRepository::instantiateWithoutMonitorThreadForTests();

TEST(ThreadMonitor, MemoryOverhead) {
  using Collection =
      plf::colony<ThreadMonitorCentralRepository::ThreadRegistration>;
...
TEST(CentralRepository, ThreadTimeout) {
  const auto frozenCount = ThreadMonitorCentralRepository::instance()
    ->getLivenessErrorConditionDetectedCount();
  ThreadMonitorCentralRepository::instance()->setThreadTimeout(
      std::chrono::milliseconds{1});
  std::atomic<bool> livenessConditionDetected = false;
  ThreadMonitorCentralRepository::instance()
    ->setLivenessErrorConditionDetectedCallback([&] {
  ...
  ASSERT_EQ(frozenCount + 1, ThreadMonitorCentralRepository::instance()
    ->getLivenessErrorConditionDetectedCount());
}
...
TEST(CentralRepository, CentralRepositoryUpdates) {
  ...
  ThreadMonitorCentralRepository::instance()->setReportingInterval(
      std::chrono::milliseconds{1});

  ThreadMonitor<> monitor("test", 1);
  auto initialStates = ThreadMonitorCentralRepository::instance()
      ->getAllThreadLivenessStates();
  ASSERT_EQ(1, initialStates.size());
  while (true) {
    threadMonitorCheckpoint(2);
    std::this_thread::sleep_for(std::chrono::milliseconds{1});
    auto states = ThreadMonitorCentralRepository::instance()
      ->getAllThreadLivenessStates();
    ASSERT_EQ(1, initialStates.size());
